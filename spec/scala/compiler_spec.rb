...
shared_examples "ScalacCompiler" do
  ...
  it 'should package .class into a .jar file' do
    ...
    Zip::ZipFile.open(project('test1').package(:jar).to_s) do |zip|
      expect(zip.file.exist?('com/example/Test1.class')).to be_truthy
    end
  end
  ...
  it 'should compile scala class depending on java class in same project' do
    ...
    expect(file('target/test1-1.0.jar')).to exist
    Zip::ZipFile.open(project('test1').package(:jar).to_s) do |zip|
      expect(zip.file.exist?('com/example/Foo.class')).to be_truthy
      expect(zip.file.exist?('com/example/Bar.class')).to be_truthy
    end
  end
